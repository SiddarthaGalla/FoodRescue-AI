...
import { 
  ...
  Navigation, Target, Sparkles, SlidersHorizontal, Compass, MessageSquare, QrCode, Calendar, Repeat, Volume2, Receipt, Thermometer, Trophy
} from 'lucide-react';
...
export const NGODashboard: React.FC = () => {
  ...
  // NGO Location state
  const [ngoLocation, setNgoLocation] = useState<Coordinates | null>({ lat: 28.6139, lng: 77.2090 });
  const [ngoAddress, setNgoAddress] = useState<string>('Central New Delhi');
  const [isDetectingLocation, setIsDetectingLocation] = useState<boolean>(false);
  const [showLocationModal, setShowLocationModal] = useState<boolean>(false);
  const [customLat, setCustomLat] = useState<string>('28.6139');
  const [customLng, setCustomLng] = useState<string>('77.2090');
  const [customAddr, setCustomAddr] = useState<string>('Central New Delhi');
...
const DUMMY_NGO_DONATIONS: Donation[] = [
  ...
  },
  {
    ...
    status: 'available',
    pickupLocation: 'Jubilee Hills Rd #36, Hyderabad (Geo: 17.4325° N, 78.4071° E)',
    latitude: 17.4325,
    longitude: 78.4071,
  ...
  {
    id: 'DON-9480',
    title: 'Steamed Rice & Lentil Curry Casseroles',
    description: 'Nutritious dal tadka and basmati rice portions stored in thermal food warmers.',
    quantity: 120,
    itemType: 'Cooked Meals',
    status: 'claimed',
    pickupLocation: 'Connaught Place Block B, New Delhi (Geo: 28.6315° N, 77.2167° E)',
    latitude: 28.6315,
    longitude: 77.2167,
    ...
    donorPhone: '+91 98999 12345',
    temperature: '65°C (Safe Hot Hold)',
    foodSafetyPassed: true,
    claimedByNgoName: 'Hope Community Shelter',
    volunteerName: 'Driver Fleet #402 (Rahul V.)',
    photoUrl: 'https://images.unsplash.com/photo-1546833999-b9f581a1996d?w=500&auto=format&fit=crop&q=80',
